class WatchlistManager {
   ...
   async activateList(listId) {
    ...
    // ✅ Сохраняем сортировку для текущего списка ПЕРЕД переключением
    ...
    // ✅ ПЕРЕНЕС СЮДА - Восстанавливаем сортировку для нового списка ПОСЛЕ создания тикеров
...
// ============================================
// ДОБАВИТЬ ЭТИ МЕТОДЫ В КЛАСС WATCHLISTMANAGER
// ============================================
...
_restoreSortForList(listId) {
    ...
    // Обновляем иконки вручную без вызова setupHeaderSorting
...
__restoreSortForList(listId) {
    ...
    // ❌ УБЕРИ ЭТИ СТРОКИ - ОНИ ПОРТЯТ ВСЁ!
    // if (this.tickerPanel.renderer?.setupHeaderSorting) {
    //     this.tickerPanel.renderer.setupHeaderSorting();
    // }
    
    // ✅ ВМЕСТО ЭТОГО - ПРОСТО ОБНОВИ ИКОНКИ
    this._updateHeaderIcons();
...
_updateHeaderIcons() {
  ...
  async fetchPricesForActiveList() {
    ...
    const BATCH = 20;
    const DELAY = 1000; // ⬅️ 1 СЕКУНДА МЕЖДУ БАТЧАМИ
    ...
    for (let i = 0; i < bnFut.length; i += BATCH) {
        ...
        // ✅ ВСЕГДА ЖДЕМ МЕЖДУ БАТЧАМИ, ДАЖЕ ЕСЛИ ЭТО ПОСЛЕДНИЙ
        if (i + BATCH < bnFut.length) {
            await new Promise(r=>setTimeout(r, DELAY));
        }
    }

    // Binance Spot
    if (bnFut.length > 0 && bnSpot.length > 0) {
        await new Promise(r=>setTimeout(r, 2000)); // ⬅️ 2 СЕКУНДЫ МЕЖДУ FUTURES И SPOT
    }
    ...
    for (let i = 0; i < bnSpot.length; i += BATCH) {
        ...
        if (i + BATCH < bnSpot.length) {
            await new Promise(r=>setTimeout(r, DELAY));
        }
    }

    // Bybit (без задержки, он не банит)
