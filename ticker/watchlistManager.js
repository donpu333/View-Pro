class WatchlistManager {
    ...
    async activateList(listId) {
        ...
        // ✅ Цены подтянутся через WebSocket и pollRestData (без спама API)
    }
    ...
    async addSymbolToList(listId, symbol, exchange, marketType) {
        ...
        if (listId === this.activeListId) {
            ...
            // ✅ Не вызываем fetchPricesForActiveList — цены придут через WS
        }
    ...
    // ✅ ИСПРАВЛЕНО: батчевые запросы только нужных тикеров
    ...
    async initializeWithPriority() {
        ...
        // ✅ Не вызываем fetchPricesForActiveList при инициализации — цены придут через WS и pollRestData
    }
