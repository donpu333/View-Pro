...
_deduplicateSymbols(symbols) {
 ...
 addInitialSymbols() {
    ...
    // ✅ ТОЛЬКО BYBIT (он не банит)
    const hasBybitFutures = [...this.tickersMap.keys()].some(k => k.includes(':bybit:futures'));
    const hasBybitSpot = [...this.tickersMap.keys()].some(k => k.includes(':bybit:spot'));
    
    const promises = [];
    if (hasBybitFutures) {
        promises.push(fetch('https://api.bybit.com/v5/market/tickers?category=linear').then(r => r.json()).catch(() => null));
    }
    if (hasBybitSpot) {
        promises.push(fetch('https://api.bybit.com/v5/market/tickers?category=spot').then(r => r.json()).catch(() => null));
    }
    
    if (promises.length > 0) {
        Promise.all(promises).then(([byF, byS]) => {
            if (byF?.retCode === 0) {
                byF.result.list.forEach(t => {
            ...
            // Рендерим с уже полученными ценами Bybit
        ...
        // Нет Bybit символов — просто рендерим
        this.renderTickerList();
        requestAnimationFrame(() => {
            const container = document.getElementById('tickerListContainer');
            const loader = document.getElementById('tickerLoader');
