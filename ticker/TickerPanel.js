...
_restoreWebSockets() {
    ...
    // Также запускаем pollRestData если нужно
...
this._wsUpdateRafId = null;
    ...
    const loadAllData = async () => {
        ...
        if (!this._blockDOMUpdates) {
            // ✅ Откладываем тяжелую сортировку и рендер на 50мс, 
            // чтобы разбить долгую задачу (Long Task) и не фризить график
            setTimeout(() => {
                this.filterCache = null; 
                this.renderTickerList();
                this.updateModalCount?.();
    ...
    if (!self._wsUpdateRafId) {
        self._wsUpdateRafId = requestAnimationFrame(() => {
            self.renderer.updatePriceElements();
            self._flashUpdatedRows(Date.now());
            self._wsUpdateRafId = null;
        });
    }
}
            } catch(e) {}
