...
class HorizontalRayRenderer {
    ...
    hitTest(x, y) {
        if (this._hitArea) {
            ...
            // FIX: универсальная проверка ширины холста (без привязки к Mac)
...
class AlertLineRenderer {
    constructor(alert, chartManager) {
        ...
        this._hitArea = null;
        this._priceLabelHitArea = null;
        // FIX: убрана проверка на Mac, оставлен только pixelRatio для универсальности
        this._pixelRatio = window.devicePixelRatio || 1;
    }
    ...
    // FIX: Универсальный hitTest без привязки к ОС
    hitTest(x, y) {
        if (this._hitArea) {
            const buffer = 15;
            const inY = Math.abs(y - this._hitArea.y - this._hitArea.height / 2) < (this._hitArea.height / 2 + buffer);
