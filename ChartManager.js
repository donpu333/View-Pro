class ChartManager {
  constructor(container) {
    ...
    this._priceTimerRafId = null;
    this._priceTimerGeometryDirty = true;
    this._priceTimerContainerRect = null;
    this._priceTimerHeight = 0;
    this._windowInnerHeight = window.innerHeight;
    this._lastPriceTimerTop = null;
    this._lastPriceTimerBullish = null;
    this._priceTimerRightApplied = false;
    this._priceTimerResizeObserver = null;
    this._windowScrollHandler = null;
    this._priceTimerScrollAttached = false;
    ...
    if (this.timerManager) {
      this.timerManager.updatePosition = () => this._invalidatePriceTimerGeometry();
    }
    ...
    this._ensurePriceTimerObservers();
    this._startPriceTimerLoop();
  ...
  destroy() {
    this._stopPriceTimerLoop();

    if (this._priceTimerResizeObserver) {
      this._priceTimerResizeObserver.disconnect();
      this._priceTimerResizeObserver = null;
    }

    if (this._windowScrollHandler) {
      window.removeEventListener('scroll', this._windowScrollHandler);
      window.removeEventListener('orientationchange', this._windowScrollHandler);
      this._windowScrollHandler = null;
    }
  ...
  setupEventListeners() {
    ...
    if (!this._priceTimerScrollAttached) {
      this._windowScrollHandler = () => this._invalidatePriceTimerGeometry();
      window.addEventListener('scroll', this._windowScrollHandler, { passive: true });
      window.addEventListener('orientationchange', this._windowScrollHandler);
      this._priceTimerScrollAttached = true;
    }
  }
  ...
  scheduleUpdatePosition() {
    this._invalidatePriceTimerGeometry();
  }

  updatePriceLineTimerPosition() {
    this._updatePriceTimerPositionFrame();
  }
  ...
  _invalidatePriceTimerGeometry() {
    this._priceTimerGeometryDirty = true;
  }

  _ensurePriceTimerObservers() {
    if (!this.priceLineTimer) {
      this.priceLineTimer = this._priceLineTimer || document.getElementById('priceLineTimer');
      if (!this.priceLineTimer) return false;
      this._invalidatePriceTimerGeometry();
    }

    if (!this._priceTimerResizeObserver && typeof ResizeObserver !== 'undefined') {
      this._priceTimerResizeObserver = new ResizeObserver(() => this._invalidatePriceTimerGeometry());
      this._priceTimerResizeObserver.observe(this.priceLineTimer);
    }

    return true;
  }

  _startPriceTimerLoop() {
    if (this._priceTimerRafId !== null) return;

    const tick = () => {
      this._priceTimerRafId = requestAnimationFrame(tick);
  ...
  _stopPriceTimerLoop() {
    if (this._priceTimerRafId !== null) {
      cancelAnimationFrame(this._priceTimerRafId);
      this._priceTimerRafId = null;
    }
  }

  _updatePriceTimerPositionFrame() {
    if (!this._ensurePriceTimerObservers()) return;
    if (!this._isChartValid() || !this.lastCandle) return;
    ...
    if (this._priceTimerGeometryDirty || !this._priceTimerContainerRect) {
      this._priceTimerContainerRect = this.chartContainer.getBoundingClientRect();
      this._priceTimerHeight = this.priceLineTimer.offsetHeight || 30;
      this._windowInnerHeight = window.innerHeight;
      this._priceTimerGeometryDirty = false;
    }

    const coordinate = activeSeries.priceToCoordinate(price);
    if (coordinate === null || isNaN(coordinate)) return;

    let top = coordinate + this._priceTimerContainerRect.top;

    const minTop = 5;
    const maxTop = Math.max(minTop, this._windowInnerHeight - this._priceTimerHeight - 5);

    top = Math.max(minTop, Math.min(maxTop, top));

    const topPx = `${Math.round(top * 10) / 10}px`;

    if (this._lastPriceTimerTop !== topPx) {
      this.priceLineTimer.style.top = topPx;
      this._lastPriceTimerTop = topPx;
    }

    if (!this._priceTimerRightApplied) {
      this.priceLineTimer.style.right = '10px';
      this._priceTimerRightApplied = true;
    }

    const isBullish = this.lastCandle ? (this.lastCandle.close >= this.lastCandle.open) : true;

    if (this._lastPriceTimerBullish !== isBullish) {
      this.priceLineTimer.classList.remove('bullish', 'bearish');
      this.priceLineTimer.classList.add(isBullish ? 'bullish' : 'bearish');
      this._lastPriceTimerBullish = isBullish;
    }
  }
}
