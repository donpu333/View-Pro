...
class ChartManager {
    constructor(container) {
        ...
        this._pendingPriceUpdate = null; // { price, time }
    ...
    // Для дневных и старше — формат ДД.ММ в UTC (без сдвига)
    ...
    // Для внутридневных — часы:минуты по Москве (как было)
    return date.toLocaleTimeString('ru-RU', {
        ...
        // Настройка price line для основных серий
        ...
        // Применяем настройки шкалы объёма
    ...
    _applyVolumeScaleOptions() {
        ...
        console.log('✅ Volume scale options applied:', this._volumeScaleMargins);
    }
    ...
    _isFresherUpdate(existingCandle, receivedAt, source) {
        ...
        // Тай-брейк при абсолютном совпадении времени (коллизия миллисекунд):
        // WS (3) всегда побеждает REST (2), а REST побеждает Cache (1).
    ...
    _getNextIntervalTime(timeSec) {
        ...
        // Для 1d и меньше арифметика секунд в UTC абсолютно точна и быстрее
        ...
        try {
            ...
            for (let i = currentData.length - 1; i >= Math.max(0, currentData.length - 3); i--) {
                ...
                if (freshCandle) {
                    ...
                    // Если текущая свеча уже закрыта, не обновляем её из фоновой синхронизации
        ...
        try {
            ...
            for (const freshCandle of freshCandles) {
                ...
                if (existing) {
                    ...
                    if (existing._closed === true && freshCandle._closed !== true) continue; // защита от перезаписи закрытых
            ...
            if (!hasStructuralChange) {
                if (lastCandleFresh) {
                    ...
                    // Не обновляем закрытую последнюю свечу из фоновой синхронизации, если только она не подтверждает закрытие
    ...
    async _catchUpMissedCandles() {
        ...
        try {
            ...
            if (newCandles.length > 0) {
                ...
                // ФИКС: обновляем ОБЕ серии (candle и bar), а не только активную,
                // чтобы при переключении типа графика не было рассинхрона данных.
                if (this.candleSeries) this.candleSeries.setData(this.chartData);
                if (this.barSeries) this.barSeries.setData(this.chartData);
    ...
    _applyPriceUpdate(price, tickTime = null) {
        ...
        if (lastCandle.time !== currentCandleStart) {
            ...
            if (existingIndex !== undefined) {
                ...
                // ГЛАВНЫЙ ФИКС: если разрыв больше одного интервала —
                // placeholder НЕ создаём, иначе он заблокирует догрузку.
                const expectedNextTime = this._getNextIntervalTime(lastCandle.time);

                if (currentCandleStart > expectedNextTime) {
                    // Разрыв — запускаем догрузку, свечу пока не рисуем
                ...
                // Обычная следующая свеча — создаём placeholder
                const newCandle = {
    ...
    if (this.currentInterval === '1w') {
        // ─────────────────────────────────────────────────────────────
        // ПРАВИЛЬНО: отступаем НАЗАД к понедельнику 00:00:00 UTC
        // текущей недели.
        //
        // getUTCDay(): 0=Вс, 1=Пн, ..., 6=Сб
        // (dayOfWeek + 6) % 7 даёт количество дней с понедельника:
        //   Пн → 0, Вт → 1, ..., Сб → 5, Вс → 6
        // ─────────────────────────────────────────────────────────────
    ...
    updateLastCandle(candle, eventTime = null, meta = null) {
        ...
        const nowSec = Math.floor(Date.now() / 1000);
        const maxAllowedTime = this._getNextIntervalTime(this._alignTimeToInterval(nowSec));
        ...
        try {
            ...
            // Определяем, должна ли свеча считаться закрытой:
            //   - явный isClosed из WS (k.x === true / confirm === true), ИЛИ
            //   - время свечи меньше начала текущего интервала (свеча уже точно в прошлом).
            const currentCandleStart = this._alignTimeToInterval(nowSec);
            ...
            if (isLastCandle) {
                // Блокируем нефинальные обновления уже закрытой свечи
                ...
                // Аналогично: не перезаписываем закрытую свечу нефинальным обновлением
                ...
                if (currentLastCandle) {
                    ...
                    if (candle.time > expectedNextTime) {
                        // Разрыв — не добавляем, сначала догрузим пропущенные
                        setTimeout(() => {
                            this._catchUpMissedCandles().catch(() => {});
                        }, 100);
                        return;
    ...
    setDataQuick(data, interval, symbol, exchange = 'binance', marketType = 'futures', forceNewSymbol = false, onReady = null) {
        try {
            ...
            // Помечаем свечи как закрытые, если их время меньше начала текущего интервала
            const currentCandleStart = this._alignTimeToInterval(Date.now() / 1000);
            data.forEach(c => {
    ...
    _subscribeToPrice() {
        // ФИКС: раньше при отсутствующем priceManager retry зацикливался навсегда,
        // т.к. повторно проверял только this.priceManager, а не window.priceManagerInstance.
        if (!this.priceManager) {
    ...
    _performTrimNow(fromIndex, toIndex) {
        ...
        try {
            ...
            // ГЛАВНЫЙ ФИКС: раньше здесь обрезался только внутренний chartData,
            // а candleSeries/barSeries продолжали хранить старый, необрезанный набор данных.
            // Это приводило к рассинхрону между свечами и объёмом (volumeSeries ниже
            // получал уже обрезанные данные) и к неверному сдвигу видимого диапазона.
            if (this.candleSeries) this.candleSeries.setData(this.chartData);
            if (this.barSeries) this.barSeries.setData(this.chartData);
        ...
        try {
            ...
            if (matchLast && this._isFresherUpdate(this.chartData[this.chartData.length - 1], matchLast._receivedAt, matchLast._source)) {
                ...
                if (lc._closed === true && matchLast._closed !== true) {
                    ...
                    // ФИКС: обновляем ОБЕ серии, а не только активную, чтобы неактивная
                    // серия не показывала устаревшую последнюю свечу после переключения типа графика.
                    const updateData = {
