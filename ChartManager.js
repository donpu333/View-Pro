...
if (lastLoaded && lastLoaded.time < currentCandleStart) {
const expectedNext = this._getNextIntervalTime(lastLoaded.time);
if (currentCandleStart === expectedNext) {
const placeholderPrice = this.getCurrentPrice() ?? this.currentRealPrice ?? lastLoaded.close;
if (typeof placeholderPrice === 'number' && !isNaN(placeholderPrice) && placeholderPrice > 0) {
data.push({
time: currentCandleStart,
...
} else if (currentCandleStart > expectedNext) {
setTimeout(() => {
this._catchUpMissedCandles().catch(() => {});
}, 0);
}
