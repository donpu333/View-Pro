...
_stampCandle(candle, source) {
candle._source = source;
candle._receivedAt = Date.now();
return candle;
}
_isCandleFresher(existingCandle, newCandle, newSource) {
const existingSource = existingCandle._source || 'unknown';
const existingReceivedAt = existingCandle._receivedAt || 0;
const newPriority = this.SOURCE_PRIORITY[newSource] || 0;
const existingPriority = this.SOURCE_PRIORITY[existingSource] || 0;
if (newPriority !== existingPriority) {
return newPriority > existingPriority;
}
const newReceivedAt = newCandle._receivedAt || Date.now();
return newReceivedAt > existingReceivedAt;
}
// ============ МЕТОДЫ ЦВЕТА ============
...
async _syncRecentCandles(ignoreSwitching = false) {
const genId = this._activeGeneration;
this._syncRequestStartTime = Date.now();
try {
...
if (!ignoreSwitching && (this._updatesSuspended || this._switchingSymbol || this._activeGeneration !== genId)) return;
if (ignoreSwitching && this._activeGeneration !== genId) return;
...
const lastKlineTime = this._lastKlineEventTime || 0;
const requestStart = this._syncRequestStartTime || 0;
if (lastKlineTime > requestStart) {
continue;
}
}
if (!this._isCandleFresher(cur, freshCandle, 'rest')) {
continue;
}
...
if (!this._isCandleFresher(existing, freshCandle, 'rest')) {
updatedData.push(existing);
} else {
const stamped = this._stampCandle({...freshCandle}, 'rest');
updatedData.push(stamped);
dataChanged = true;
}
} else {
updatedData.push(existing);
...
lastCandle._source = 'ws';
lastCandle._receivedAt = Date.now();
this.currentRealPrice = price;
...
// 🛡️ ПРАВКА 1: Добавлен параметр meta и жесткий фаервол от призрачных WS-сообщений
updateLastCandle(candle, eventTime = null, meta = {}) {
    // 🛡️ ATOMIC SWITCH: Абсолютный фаервол от опоздавших WS-пакетов
    if (meta.symbol && meta.symbol !== this.currentSymbol) return;
    if (meta.interval && meta.interval !== this.currentInterval) return;
    ...
    try {
        ...
        if (isLastCandle) {
            if (!this._isCandleFresher(currentLastCandle, candle, 'ws')) {
                return;
            }
            ...
            this._stampCandle(currentLastCandle, 'ws');
            ...
            if (!this._isCandleFresher(existingCandle, candle, 'ws')) {
                return;
            }
            ...
            this._stampCandle(existingCandle, 'ws');
            if (this._isChartValid()) {
                if (this.candleSeries) this.candleSeries.setData(this.chartData);
                if (this.barSeries) this.barSeries.setData(this.chartData);
                if (this.volumeSeries) {
...
const source = forceNewSymbol ? 'rest' : 'cache';
for (const candle of data) {
this._stampCandle(candle, source);
}
this.chartData = data;
this._candleTimeMap.clear();
...
// 🛡️ ПРАВКА 2: finishSwitch синхронный — оверлей снимается мгновенно, REST идёт фоном
...
chartData: this.chartData.slice(),
lastCandle: this.lastCandle,
currentRealPrice: this.currentRealPrice,
};
this.currentRealPrice = null;
this.lastCandle = null;
...
// ✅ ИСПРАВЛЕНИЕ БАГА ТУСКЛОСТИ: finishSwitch синхронный
// Оверлей снимается мгновенно после setDataQuick, REST-синхронизация идёт фоном
const finishSwitch = () => {
    if (this._activeGeneration !== generationId) return;
    
    // REST-синхронизация идёт фоном, НЕ блокируя UI
    this._syncRecentCandles(true).catch(() => {});
    
    this._switchingSymbol = false;
    this._resumeAllUpdates(generationId);
    this._hideSymbolSwitchOverlay(); // Мгновенно убираем затемнение
    
    if (window.wsManager?.subscribe) {
        window.wsManager.subscribe(symbol, this.currentInterval, exchange, marketType);
    }
...
if (window.wsManager?.unsubscribe) {
window.wsManager.unsubscribe(this.currentSymbol, this.currentInterval, this.currentExchange, this.currentMarketType);
}
this.chartData = [];
this._candleTimeMap.clear();
this._lastKlineEventTime = 0;
...
this.currentSymbol = previousState.symbol;
this.currentExchange = previousState.exchange;
this.currentMarketType = previousState.marketType;
this.chartData = previousState.chartData;
this.lastCandle = previousState.lastCandle;
this.currentRealPrice = previousState.currentRealPrice;
this._rebuildTimeMap();
if (this._isChartValid() && this.chartData.length > 0) {
this.setDataQuick(this.chartData, this.currentInterval, this.currentSymbol, this.currentExchange, this.currentMarketType, true, finishSwitch);
} else {
finishSwitch();
}
...
if (isFromCache) {
const genId = this._activeGeneration;
this.refreshCandlesInBackground(symbol, exchange, marketType, this.currentInterval, genId).catch(() => {});
}
} catch (error) {
...
// 🛡️ ПРАВКА 3: Усилена проверка контекста в подписке на цену
_subscribeToPrice() {
if (!this.priceManager) {
...
this._priceUpdateHandler = (price, symbol, exchange, marketType) => {
    // 🛡️ ATOMIC SWITCH: Жесткая изоляция — дропаем всё, что не от текущего тикера
...
async fetchKlines(symbol, exchange, marketType, interval, limit = 1000, endTime = null, requestType = 'user', externalSignal = null) {
let controller;
...
const signal = externalSignal ? externalSignal : controller.signal;
...
async refreshCandlesInBackground(symbol, exchange, marketType, interval, generationId = null) {
const genId = generationId || this._activeGeneration;
try {
...
if (this._isCandleFresher(lc, matchLast, 'background')) {
...
this._stampCandle(lc, 'background');
if (activeSeries) {
...
this._stampCandle(c, 'background');
if (activeSeries) {
