            ...
            try {
                ...
                if (self.currentExchange === 'binance' && raw.stream) {
                    ...
                    if (raw.stream.includes('@kline')) {
                        ...
                        if (!isNaN(price)) {
                            // ✅ Обновляем title СРАЗУ (Worker не заморожен)
                            self.chartManager.currentRealPrice = price;
                            if (self.chartManager._updatePageTitle) {
                                self.chartManager._updatePageTitle();
                            }
                            // График обновляем как обычно
