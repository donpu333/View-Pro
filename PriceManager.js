class PriceManager {
   ...
   _init() {
    ...
    // ✅ Пинг каждые 15 секунд — не даёт WebSocket уснуть в фоне
    ...
    // ✅ При возврате на вкладку — принудительная выгрузка и переподключение если нужно
    if (typeof document !== 'undefined') {
        document.addEventListener('visibilitychange', () => {
            if (!document.hidden) {
                // Выгружаем накопленные обновления
                if (this._pendingUpdates.size > 0) {
                    this._forceFlushUpdates();
                }
                // Проверяем состояние вебсокетов, переподключаем если умерли
                for (const [key, ws] of Object.entries(this.connections)) {
                ...
                // Запрашиваем свежие REST-цены для активных алертов
                this._pollAlertPricesViaRest();
            }
        });
    }
    
    console.log('✅ PriceManager v12 запущен (Real-time + keep-alive + фон)');
}
...
const change = (parseFloat(ticker.price24hPcnt) || 0) * 100;
 ...
 _setPrice(symbol, priceData, exchange, marketType) {
    ...
    if (this._flushRafId === null) {
        if (document.hidden) {
            // ✅ В ФОНЕ: setTimeout вместо RAF (RAF заморожен браузером)
            this._flushRafId = setTimeout(() => {
                this._flushRafId = null;
                this._forceFlushUpdates();
            }, 250);
        } else {
            // ✅ АКТИВНАЯ ВКЛАДКА: RAF как обычно
            this._flushRafId = requestAnimationFrame(() => {
                this._flushRafId = null;
                this._forceFlushUpdates();
            });
