...
class VolumeProfileFixedRangeIndicator extends BaseIndicator {
    constructor(manager) {
        super(manager, 'volprofile', 'Volume Profile', '#2962FF', 'main');
        ...
        this.settings = {
            ...
            valueAreaPercent: 70,
            pocColor: '#ff0000',
            valueAreaUpColor: 'rgba(33, 150, 243, 0.3)',
            valueAreaDownColor: 'rgba(255, 152, 0, 0.3)',
            upColor: 'rgba(33, 150, 243, 0.15)',
            downColor: 'rgba(255, 152, 0, 0.15)',
            showPoc: true
        };
        ...
        this._onDataUpdateBound = null;
        this._onScaleChangeBound = null;
        
        this._createEmptySeries();
        this._initCanvasOverlay();
        this._setupChartListeners();

        // 🛠 Принудительный первичный расчёт, если данные уже есть
        if (this.manager.chartManager.chartData?.length) {
            this.updateSeriesData(this.manager.chartManager.chartData);
        } else {
            // Ожидание асинхронной подгрузки данных
            const checkData = () => {
                if (this.manager.chartManager.chartData?.length) {
                    this.updateSeriesData(this.manager.chartManager.chartData);
                } else {
    ...
    _initCanvasOverlay() {
        const chartContainer = this.manager.chartManager.chartContainer 
                            || document.getElementById('chart-container');
        if (!chartContainer) {
            console.warn('Volume Profile: chartContainer не найден, канвас не создан');
            return;
        }

        this._canvas = document.createElement('canvas');
        this._canvas.id = 'vp-canvas-overlay';
    ...
    _getVol(y11, y12, y21, y22, height, vol) {
        const max1 = Math.max(y11, y12);
        const min1 = Math.min(y11, y12);
        const max2 = Math.max(y21, y22);
        const min2 = Math.min(y21, y22);
        const intersection = Math.max(Math.min(max1, max2) - Math.max(min1, min2), 0);
        return height > 0 ? (intersection * vol / height) : 0;
    }

    _calculateProfile(data) {
        const { numBars, rowSize } = this.settings;
        const slice = data.slice(-numBars);
        if (slice.length < 2) return null;

        let top = -Infinity, bot = Infinity;
        slice.forEach(c => { top = Math.max(top, c.high); bot = Math.min(bot, c.low); });
        if (top === bot) return null;

        const step = (top - bot) / rowSize;
        const levels = [];
        for (let x = 0; x <= rowSize; x++) levels.push(bot + step * x);

        const bins = levels.slice(0, -1).map((low, i) => ({
            low, high: levels[i + 1],
            totalVol: 0, upVol: 0, downVol: 0
        }));
        ...
        slice.forEach(candle => {
            ...
            bins.forEach(bin => {
                const binVol = this._getVol(candle.low, candle.high, bin.low, bin.high, candleHeight, vol);
                bin.totalVol += binVol;
                if (isUp) bin.upVol += binVol;
                else bin.downVol += binVol;
            });
        });

        let poc = bins[0];
        bins.forEach(b => { if (b.totalVol > poc.totalVol) poc = b; });

        const totalVolume = bins.reduce((s, b) => s + b.totalVol, 0);
        const targetVaVol = totalVolume * (this.settings.valueAreaPercent / 100);
        let vaVol = poc.totalVol;
        let vaHigh = poc.high, vaLow = poc.low;
        let upIdx = bins.indexOf(poc), downIdx = bins.indexOf(poc);

        while (vaVol < targetVaVol && (upIdx + 1 < bins.length || downIdx - 1 >= 0)) {
            const upNext = upIdx + 1 < bins.length ? bins[upIdx + 1].totalVol : 0;
            const downNext = downIdx - 1 >= 0 ? bins[downIdx - 1].totalVol : 0;

            if (upNext >= downNext) {
                upIdx++; vaVol += upNext; vaHigh = bins[upIdx].high;
            } else {
                downIdx--; vaVol += downNext; vaLow = bins[downIdx].low;
            }
        }

        return { bins, poc, vaHigh, vaLow };
    }

    _drawProfile() {
        if (!this._canvas || !this._ctx || !this._vpData || !this.visible || this._isDrawing) return;
        ...
        const timeRight = visibleRange.to;
        const rightX = chart.timeScale().timeToCoordinate(timeRight);
        
        if (rightX === null) { this._isDrawing = false; return; }

        const maxPocVol = this._vpData.poc.totalVol;

        this._vpData.bins.forEach(bin => {
            const yTop = chart.priceScale('right').priceToCoordinate(bin.high);
            const yBot = chart.priceScale('right').priceToCoordinate(bin.low);
            
            if (yTop === null || yBot === null || yTop < 0 || yBot > rect.height) return;

            const height = Math.abs(yBot - yTop);
            if (height < 1) return;

            let color = (bin.upVol > bin.downVol) ? this.settings.upColor : this.settings.downColor;
            
            if (bin.low >= this._vpData.vaLow && bin.high <= this._vpData.vaHigh) {
                color = (bin.upVol > bin.downVol) ? this.settings.valueAreaUpColor : this.settings.valueAreaDownColor;
            }

            const width = Math.max((bin.totalVol / maxPocVol) * 80, 2);

            this._ctx.fillStyle = color;
            this._ctx.fillRect(rightX - width, yTop, width, height);
        });

        this._ctx.font = "bold 10px 'JetBrains Mono', monospace";
        this._ctx.textBaseline = 'middle';

        if (this.settings.showPoc) {
            const yPoc = chart.priceScale('right').priceToCoordinate(this._vpData.poc.high);
            if (yPoc !== null && yPoc > 10 && yPoc < rect.height - 10) {
                this._ctx.beginPath();
                this._ctx.moveTo(0, yPoc);
                this._ctx.lineTo(rightX, yPoc);
                this._ctx.strokeStyle = this.settings.pocColor;
                this._ctx.lineWidth = 2;
                this._ctx.setLineDash([5, 5]);
                this._ctx.stroke();
                this._ctx.setLineDash([]);

                const pocText = `POC: ${this._vpData.poc.high.toFixed(2)}`;
                const textWidth = this._ctx.measureText(pocText).width;
                this._ctx.fillStyle = this.settings.pocColor;
                this._ctx.fillText(pocText, rightX - textWidth - 5, yPoc);
            }
        }

        const drawVALabel = (price, label) => {
            const yCoord = chart.priceScale('right').priceToCoordinate(price);
            if (yCoord === null || yCoord < 10 || yCoord > rect.height - 10) return;
            
            this._ctx.fillStyle = '#ffffff';
            this._ctx.fillText(label, 5, yCoord);
        };

        drawVALabel(this._vpData.vaHigh, `VAH: ${this._vpData.vaHigh.toFixed(2)}`);
        drawVALabel(this._vpData.vaLow, `VAL: ${this._vpData.vaLow.toFixed(2)}`);
    ...
    _setupChartListeners() {
        ...
        this._onDataUpdateBound = () => this.updateSeriesData(this.manager.chartManager.chartData);
        if (chart.on) chart.on('dataUpdate', this._onDataUpdateBound);

        this._onScaleChangeBound = () => requestAnimationFrame(() => this._drawProfile());
        if (chart.timeScale().subscribeVisibleLogicalRangeChange) {
            chart.timeScale().subscribeVisibleLogicalRangeChange(this._onScaleChangeBound);
        }
    }

    updateSeriesData(data) {
        if (!data || data.length < this.settings.numBars) return;
        this._vpData = this._calculateProfile(data);
        this._drawProfile();
    }
    ...
    set visible(value) {
        this._visible = value;
        if (this._canvas) this._canvas.style.display = value ? 'block' : 'none';
        if (value) this._drawProfile();
        if (this.manager) this.manager._saveIndicators();
    }

    getSettingsHTML() {
        return `
            <div style="max-height:400px; overflow-y:auto; padding: 10px; color: #B0B0B0; font-size: 12px;">
                <div style="color:#2962FF; margin-bottom:12px; font-weight:bold;">📐 Volume Profile Settings</div>
                
                <div style="margin-bottom:8px; display:flex; align-items:center; gap:10px;">
                    <label style="width:140px;">Кол-во свечей:</label>
                    <input type="number" id="vp-numbars" value="${this.settings.numBars}" min="10" max="500" style="background:#1E1E1E; border:1px solid #404040; color:#fff; border-radius:4px; padding:4px 8px; width:80px;">
                </div>
                <div style="margin-bottom:8px; display:flex; align-items:center; gap:10px;">
                    <label style="width:140px;">Кол-во строк (Rows):</label>
                    <input type="number" id="vp-rowsize" value="${this.settings.rowSize}" min="5" max="100" style="background:#1E1E1E; border:1px solid #404040; color:#fff; border-radius:4px; padding:4px 8px; width:80px;">
                </div>
                <div style="margin-bottom:12px; display:flex; align-items:center; gap:10px;">
                    <label style="width:140px;">Value Area %:</label>
                    <input type="number" id="vp-vapercent" value="${this.settings.valueAreaPercent}" min="1" max="100" style="background:#1E1E1E; border:1px solid #404040; color:#fff; border-radius:4px; padding:4px 8px; width:80px;">
                </div>

                <div style="border-top:1px solid #333; margin-bottom: 10px;"></div>
                <div style="color:#888; margin-bottom:8px; font-size:11px;">Цвета</div>
                
                <div style="margin-bottom:8px; display:flex; align-items:center; gap:10px;">
                    <label style="width:140px;">POC Линия:</label>
                    <input type="color" id="vp-poccolor" value="${this.settings.pocColor}" style="background:none; border:none; width:30px; height:25px; cursor:pointer;">
                    <input type="checkbox" id="vp-showpoc" ${this.settings.showPoc ? 'checked' : ''} style="margin-left:10px; accent-color:#2962FF;"> <span>Показать</span>
                </div>
                <div style="margin-bottom:8px; display:flex; align-items:center; gap:10px;">
                    <label style="width:140px;">VA Up:</label>
                    <input type="color" id="vp-vaupcolor" value="#2155f3" style="background:none; border:none; width:30px; height:25px; cursor:pointer;">
                </div>
                <div style="margin-bottom:8px; display:flex; align-items:center; gap:10px;">
                    <label style="width:140px;">VA Down:</label>
                    <input type="color" id="vp-vadowncolor" value="#ff9800" style="background:none; border:none; width:30px; height:25px; cursor:pointer;">
                </div>
            </div>
        `;
    }

    _hexToRgba(hex, transparency) {
    ...
    applySettingsFromForm() {
        this.settings.numBars = parseInt(document.getElementById('vp-numbars')?.value || 150);
        this.settings.rowSize = parseInt(document.getElementById('vp-rowsize')?.value || 24);
        this.settings.valueAreaPercent = parseInt(document.getElementById('vp-vapercent')?.value || 70);
        this.settings.pocColor = document.getElementById('vp-poccolor')?.value || '#ff0000';
        this.settings.showPoc = document.getElementById('vp-showpoc')?.checked || false;
        
        this.settings.valueAreaUpColor = this._hexToRgba(document.getElementById('vp-vaupcolor')?.value || '#2155f3', 0.3);
        this.settings.valueAreaDownColor = this._hexToRgba(document.getElementById('vp-vadowncolor')?.value || '#ff9800', 0.3);
    ...
    destroy() {
        const chart = this.manager?.chartManager?.chart;
        if (chart) {
            if (this._onDataUpdateBound) chart.off('dataUpdate', this._onDataUpdateBound);
            if (this._onScaleChangeBound) {
                if (chart.timeScale().unsubscribeVisibleLogicalRangeChange) {
                    chart.timeScale().unsubscribeVisibleLogicalRangeChange(this._onScaleChangeBound);
                }
            }
        }
        if (this._resizeObserver) this._resizeObserver.disconnect();
        document.getElementById('vp-canvas-overlay')?.remove();
        super.destroy?.();
    }
}

window.IndicatorRegistry.set('volprofile', VolumeProfileFixedRangeIndicator);
