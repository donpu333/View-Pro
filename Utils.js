...
async function getPrecisionFromExchange(symbol, exchange, marketType) {
    ...
    if (PRECISION_CACHE.has(cacheKey)) return PRECISION_CACHE.get(cacheKey);
    ...
    try {
        ...
        if (exchange === 'binance') {
            ...
            if (priceFilter?.tickSize) {
                precision = Math.round(Math.log10(1 / parseFloat(priceFilter.tickSize)));
            }
            ...
            if (instrument?.priceFilter?.tickSize) {
                precision = Math.round(Math.log10(1 / parseFloat(instrument.priceFilter.tickSize)));
            }
        }
